        ...
        <div className="min-h-screen bg-gradient-to-br from-white via-slate-50 to-white dark:from-gray-950 dark:via-gray-900 dark:to-slate-900 text-slate-900 dark:text-gray-100 flex flex-col">
            ...
            <header className="h-20 lg:h-24 bg-indigo-50/85 dark:bg-gray-900/80 backdrop-blur border-b border-indigo-200/60 dark:border-gray-700 flex items-center justify-between px-4 lg:px-8 shadow-sm sticky top-0 z-30">
                ...
                <div className="flex items-center gap-2 lg:gap-3 text-sm">
                    {/* Notifications Icon: Only show if logged in and role is admin (future: add admin notifications page) */}
                    {user && user.role === 'admin' && false && (
                      <button onClick={() => navigate('/admin/notifications')} className="p-2.5 rounded-xl bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition-all duration-300 group relative overflow-hidden" title="Notifications">
                    ...
                    >
                        <div className="absolute inset-0 bg-gradient-to-r from-blue-500/20 to-purple-500/20 opacity-0 group-hover:opacity-100"></div>
                        {isDark ? (
