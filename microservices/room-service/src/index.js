...
if (!process.env.JWT_SECRET) {
  if (process.env.NODE_ENV === 'production') {
    console.error('JWT_SECRET is required for room-service');
    process.exit(1);
  }

  process.env.JWT_SECRET = 'dev_microservices_jwt_secret_change_me';
  console.warn('JWT_SECRET is not set. Using development fallback secret for room-service.');
}
...
app.use((err, req, res, next) => {
  ...
  res.status(502).json({ message: 'Room service error', error: err.message });
});
...
mongoose
  ...
  .catch((error) => {
    console.error('Room-service DB connection error:', error.message);
    process.exit(1);
  });
