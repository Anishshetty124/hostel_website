...
if (!process.env.JWT_SECRET) {
  if (process.env.NODE_ENV === 'production') {
    console.error('JWT_SECRET is required for notification-service');
    process.exit(1);
  }

  process.env.JWT_SECRET = 'dev_microservices_jwt_secret_change_me';
  console.warn('JWT_SECRET is not set. Using development fallback secret for notification-service.');
}
...
app.use((err, req, res, next) => {
  ...
  res.status(502).json({ message: 'Notification service error', error: err.message });
});
